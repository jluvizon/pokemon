...
class PokemonList extends Component{
    ...
    fetchWildPokemon(pokemonId) {
      fetch(Constants.GET_POKEMON_URL + pokemonId).then(res=>res.json())
        .then(res => {
    ...
    catchPokemon(index) {
      if(this.state.catchedPokemons.length === Constants.MAX_CATCHED_POKEMONS) return;

      let catchedPokemon = this.state.wildPokemons[index]
      this.state.catchedPokemons.push(catchedPokemon)
      this.setState(prevState => ({
        wildPokemons: prevState.wildPokemons.filter(pokemon => pokemon.id !== catchedPokemon.id),
    ...
    releasePokemon(index) {
      ...
      this.setState(prevState => ({
        catchedPokemons: prevState.catchedPokemons.filter(pokemon => pokemon.id !== releasedPokemon.id),
      ...
      return  <div className="row">
                <div className="col-sm-5">
                  ...
                  <ul className="row align-items-center list-unstyled">
                      {wildPokemons.map((pokemon,index)=><WildPokemon id={index} key={pokemon.id} pokemon={pokemon}
                ...
                <div className="col-sm-5">
                  ...
                  <ul className="row list-unstyled">
                      {catchedPokemons.map((pokemon,index)=><CatchedPokemon id={index} key={pokemon.id} pokemon={pokemon}
